...
type model_getDistributionByAssociationTypeId struct {
	AssociationTypeId uint16 `json:"AssociationTypeId" binding:"required"`
	IsActive          uint16 `json:"IsActive" binding:"required"`
}
...
func GetDistributionByAssociationTypeId(c *gin.Context) {
	helpers.Deferring()
	...
	response = modelstruct.Service_getDistributionByAssociationTypeId()

	c.JSON(200, response)

}

func (data model_getDistributionByAssociationTypeId) Service_getDistributionByAssociationTypeId() helpers.JsonResponse {
	helpers.Deferring()
	...
	tsql := fmt.Sprint("EXEC dbo.GetDistributionByAssociationTypeId @AssociationTypeId=?, @IsActive=?")

	//log.Fatal("tsql", tsql)
	rows, err := db.Query(tsql, data.AssociationTypeId, data.IsActive)
